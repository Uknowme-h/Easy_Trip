...
const BusOperatorPage = ({ users }) => {
  ...
  return (
    <div className="p-5">
      <h2 className="text-2xl font-bold mb-4">Bus Operators</h2>
      <div className="flex justify-between items-center mb-4">
        <p>
          Showing {users.filter((user) => user.role === "bus operator").length}{" "}
          Bus Operators
        </p>
        <button
      ...
      <div>
        {users
          .filter((user) => user.role === "bus operator")
          .map((user, index) => (
            <div
              key={index}
